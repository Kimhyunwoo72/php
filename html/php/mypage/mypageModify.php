...
<?php
    ...
    if($result){
        ...
        $result = $connect -> query($sql);
        $memberInfo = $result -> fetch_array(MYSQLI_ASSOC);
            echo "<div><div class='modify'><label for='youEmail' class='label'>이메일 주소</label></div><input class='input' type='email' id='youEmail' name='youEmail' maxlength='30' placeholder='Email 주소를 입력해 주세요.' autocomplete='off' autofocus  value='".$memberInfo['youEmail']."' required></div>";
            ...
            echo "<div><div class='label_box'><label for='youIntro' class='label'>자기소개</label></div><input class='input' type='text' id='youIntro' name='youIntro' maxlength='10'placeholder='자기소개를 작성해 주세요!' autocomplete='off' value='".$memberInfo['youIntro']."' required></div>";
            echo "<div><div class='label_box'><label for='youPhoto' class='label'>파일</label></div><input class='input file' type='file' id='youPhoto' name='youPhoto'></div>";
        }
